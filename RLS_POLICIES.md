...
policies. No DELETE grants exist anywhere. `anon` receives nothing; new tables default to
...
`scripts/rls-check/10_assertions.sql` — **42 cases, 42 passed** (executed 2026-09-05 against
PostgreSQL 18.4 with the real migrations + seed; see `RLS_TEST_RESULTS.md` for the log):
...
`supabase/tests/rls_policy_tests.sql` (pgTAP, 48 assertions) mirrors the same matrix for
...
- Customer loyalty records (points/redemptions) do not exist yet; when they arrive they must reuse
  `customer_memberships`-style policies: own-rows-only for customers, business-scoped for staff.
